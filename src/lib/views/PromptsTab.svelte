...
<div class="flex flex-col gap-3 h-full overflow-y-auto p-1">
  ...
  <GlassCard padding="p-3">
    <label class="flex items-center justify-between cursor-pointer">
      <span class="text-sm text-black/70 dark:text-white/80">Enabled</span>
      <input
        type="checkbox"
        bind:checked={appState.isEnabled}
        class="w-9 h-5 rounded-full appearance-none cursor-pointer
          bg-black/15 dark:bg-white/20 checked:bg-green-500/60
        ...
        >
          <div class="flex items-center gap-2">
            <input
              ...
              class="w-4 h-4 rounded bg-black/5 dark:bg-white/10 border border-black/15 dark:border-white/20"
            />
            <span class="text-sm font-medium text-black/85 dark:text-white/90">{action.name}</span>
          </div>
          <div class="flex items-center gap-2">
            {#if action.hotkey}
              <span class="px-2 py-0.5 rounded text-xs font-mono bg-black/5 dark:bg-white/10 text-black/50 dark:text-white/50">
        ...
        {#if expandedId === action.id}
          <div class="flex flex-col gap-3 pt-2 border-t border-black/10 dark:border-white/10">
            <label class="flex flex-col gap-1">
              <span class="text-xs text-black/50 dark:text-white/50">Name</span>
              <input
                type="text"
                value={action.name}
                oninput={(e: Event) => updateAction(action.id, 'name', (e.target as HTMLInputElement).value)}
                class="bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/15 rounded-lg px-3 py-1.5 text-sm text-black/85 dark:text-white/90 outline-none focus:border-black/25 dark:focus:border-white/30"
              />
            </label>

            <label class="flex flex-col gap-1">
              <span class="text-xs text-black/50 dark:text-white/50">Hotkey</span>
              <HotkeyRecorder
                value={action.hotkey}
            ...
            <label class="flex flex-col gap-1">
              <span class="text-xs text-black/50 dark:text-white/50">System Prompt</span>
              <textarea
                value={action.system_prompt}
                oninput={(e: Event) => updateAction(action.id, 'system_prompt', (e.target as HTMLTextAreaElement).value)}
                rows={3}
                class="bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/15 rounded-lg px-3 py-1.5 text-sm text-black/85 dark:text-white/90 outline-none focus:border-black/25 dark:focus:border-white/30 resize-none"
              ></textarea>
            </label>

            <label class="flex flex-col gap-1">
              <span class="text-xs text-black/50 dark:text-white/50">User Template <span class="text-black/30 dark:text-white/30">(use {'{{text}}'} for selected text)</span></span>
              <textarea
                value={action.user_template}
                oninput={(e: Event) => updateAction(action.id, 'user_template', (e.target as HTMLTextAreaElement).value)}
                rows={2}
                class="bg-black/5 dark:bg-white/10 border border-black/10 dark:border-white/15 rounded-lg px-3 py-1.5 text-sm text-black/85 dark:text-white/90 font-mono outline-none focus:border-black/25 dark:focus:border-white/30 resize-none"
              ></textarea>
            </label>

            <label class="flex flex-col gap-1">
              <span class="text-xs text-black/50 dark:text-white/50">Output Rules</span>
              <input
                type="text"
            ...
            <div class="flex justify-between pt-1">
              <div class="flex gap-1">
                <button
                  class="px-2 py-1 rounded text-xs bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 text-black/50 dark:text-white/50"
              ...
              <button
                class="px-3 py-1 rounded text-xs bg-red-500/20 hover:bg-red-500/30 text-red-600 dark:text-red-300"
  ...
  <button
    class="w-full py-2 rounded-xl text-sm font-medium bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 text-black/50 dark:text-white/60 border border-dashed border-black/15 dark:border-white/20"
    onclick={addAction}
  >
    + Add Action
  </button>
